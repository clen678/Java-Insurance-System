...
public abstract class CliTest {
  ...
  public void runCommands(Object... commands)
    ...
    // Construct the string list of inputs/commands.
    StringBuilder sb = new StringBuilder();
    boolean first = true;

    for (Object cmdString : commands) {
      if (cmdString.getClass().isEnum() && first) {
        // the first command in the series of commands
        sb.append(cmdString.toString());
      } else if (cmdString.getClass().isEnum() && !first) {
        // a new command, but it's not the first one, so start on a new line
        sb.append(NEW_LINE);
        sb.append(cmdString.toString());
      } else if (cmdString instanceof String[]) {
        // this must be an array of values for the command's prompt(s)
        for (String value : (String[]) cmdString) {
          sb.append(NEW_LINE);
          sb.append(value);
        }
      } else {
    ...
    Object main =
        mainClass
            .getConstructor(Scanner.class, InsuranceSystem.class)
            .newInstance(new Scanner(sb.toString()), new InsuranceSystem());
